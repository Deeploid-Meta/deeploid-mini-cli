...
vsearch = plugin_manager.plugins["vsearch"] # from qiime2.plugins.vsearch.methods import join_pairs
deblur = plugin_manager.plugins["deblur"] # from qiime2.plugins.deblur.methods import denoise_16S
#quality_filter = plugin_manager.plugins["quality_filter"] # from qiime2.plugins.quality_filter.methods import q_score
...
#def quality_filtering(joined_sequences, vsearch_artifacts):
    """
    Sequence quality control and feature table construction
    This method filters sequence based on quality scores and 
    the presence of ambiguous base calls.
    """

    q_score = quality_filter.actions["q_score"]
    result = q_score(demux = joined_sequences)

    print(result)

    filtered_sequences = result.filtered_sequences
    filtered_sequences.save(str(vsearch_artifacts / 'filteredSequences.qza'))
...
def denoise(joined_sequences, deblur_artifacts):
    """
    Denoising with deblur
    Deblur requires to process a set of single end reads 
    truncated at the same length (which now should be decided after)
    """

    denoise = deblur.actions["denoise_16S"]
    result = denoise(demultiplexed_seqs=joined_sequences, trim_length=250, sample_stats=True)

    print(result)

    denoised_sequences = result.denoise_sequences
    denoised_table = result.denoise_table
    
    denoised_sequences.save(str(deblur_artifacts / 'denoised-sequences.qza'))
    denoised_table.save(str(deblur_artifacts / 'denoised-table.qza'))

   
    return denoised_sequences, denoised_table

def otu_clustering(denoised_table, denoised_sequences, vsearch_artifacts):
    """
    Creates OTUs feature table and sequences
    """

    cluster_features_de_novo = vsearch.actions["cluster_features_de_novo"]
    result = cluster_features_de_novo(sequences = denoised_sequences, table = denoised_table, perc_identity = 0.97)
...
def main():
    ...
    #filtered_sequences = quality_filtering(paired_end_sequences, output_dir)
    
    denoised_sequences, denoised_table = denoise(paired_end_sequences, output_dir)

    #dereplication
    #denoised_table, denoised_table = dereplication(sequences, output_dir)

    #OTUs (clustering)
    clustered_table, clustered_sequences = otu_clustering(denoised_table, denoised_sequences, output_dir)
